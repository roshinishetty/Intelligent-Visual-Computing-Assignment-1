...
def mlsReconstruction(points, normals, X, Y, Z):
    ...
    num_grid_points = Q.shape[0] # Caches number of grid points
    k = 50 

    # Queries the 50 nearest neighbors of each grid point in the point cloud
    tree = KDTree(points)
    dist, idx = tree.query(Q, k)

    # Computes beta as twice the average distance between each point on the surface
    # to the nearest neighbor on the surface.
    dist_between_surface_points, __ = tree.query(points, 2)
    beta = 2 * np.average(dist_between_surface_points[:,1])

    # Computing IF for each grid point
    IF = np.empty((num_grid_points))
    for grid_point_idx in np.arange(num_grid_points):
        d_phi_sum = 0
        phi_sum = 0
        for nth_nearest in np.arange(k):
            cloud_point_idx = idx[grid_point_idx][nth_nearest]
            d = np.dot(normals[cloud_point_idx], np.transpose(Q[grid_point_idx] - points[cloud_point_idx]))
            phi = np.exp(-dist[grid_point_idx][nth_nearest]**2/beta**2)
            d_phi_sum += d * phi
            phi_sum += phi
        IF[grid_point_idx] = d_phi_sum / phi_sum
    
    # Reshape IF to grid shape
    IF = IF.reshape((64, 64, 64))
...
def naiveReconstruction(points, normals, X, Y, Z):
    ...
    # Queries the nearest neighbor of each grid point in the point cloud
    tree = KDTree(points)
    __, idx = tree.query(Q, k=1)

    # For each grid point, we compute the f(p) = nj.(p-pj), for j = argmin_i ||p-pi||
    IF = np.empty((num_grid_points))
    for grid_point_idx in np.arange(num_grid_points):
        diff = Q[grid_point_idx] - points[idx[grid_point_idx]]
        IF[grid_point_idx] = np.dot(normals[idx[grid_point_idx]][0], diff[0])

    # Reshape IF to the grid shape
    IF = IF.reshape(64, 64, 64)
    ################################################
    # <================END CODE<================>
    ################################################
